...
class Portal extends CI_Controller
{
    ...
    {
        $this->load->model('OptionChoice_model');
        // get the name of step one:  Age Determination or Elemental Composition ..
    ...
    private function printTechniqueCards($tech_metas)
    {
        ...
        foreach ($array_pairs as $tech_meta_pair) {
            ...
            foreach ($tech_meta_pair as $tech_meta) {
                $models = explode('; ', $tech_meta->model);
                echo "<div class='col-sm-4'>";
                echo "<div class='card mb-3 tf-card'>";
                echo "<div class='card-header text-white bg-primary'>$tech_meta->category</div>";
                echo "<div class='card-body'>";
                foreach ($models as $model) {
                    $result = $this->OptionChoice_model->getTechniqueByModel($model);
                   
                    echo ($model != '') ? 
                    '<a href="viewGeochemAnalysis/'.$result->technique_id.'" style="text-decoration:none" class="tooltips" >'.$model.'
                    <span style="display: none; font-size: 14px !important;" class="tooltipsText">'.$result->summary.'
                    </span>
                  </a>': "";
                    echo '<br>';
                }
