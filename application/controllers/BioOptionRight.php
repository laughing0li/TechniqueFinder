<?php
/**
 * TechniqueFinder - BioOptionRight.php
 *
 * Description:
...
class BioOptionRight extends CI_Controller {
    ...
    function index(){
        $this->load->view('BioOptionRight/index');
    }

    function getList(){
        $data = array();
        $results = $this->OptionChoice_model->getAllOptionChoices('BIOLOGY', 'RIGHT');
        ...
        foreach ($results  as $r) {
            array_push($data, array(
                ...
                ."<button class='user-table-buttons' onclick=window.location='".base_url()."BioOptionRight/edit/".$r->id."'>"
                ."<span style='background: url(../assets/images/database_edit.png) 50% no-repeat;margin-right:0.4em;'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>Edit</button>"
                ."<span style='margin-left: 2em;'>&nbsp;</span>"
                ."<button class='user-table-buttons' onclick=\"if (confirm('Are you sure?')){ window.location='".base_url()."BioOptionRight/delete/".$r->id."'}\">"
    ...
    function create_new(){
        $this->load->view('BioOptionRight/new');
    }

    function edit($id){
        $optionChoice_data = $this->OptionChoice_model->getOptionChoiceById($id);
        $this->load->view('BioOptionRight/edit', array('option_data'=>$optionChoice_data));
    }
    ...
    function validate($str){
        $result = $this->OptionChoice_model->getOptionChoicesByName($str, 'BIOLOGY', 'RIGHT');
    ...
    function create(){
        ...
        if ($this->form_validation->run()) {
            ...
            $this->OptionChoice_model->createOptionChoice($name, 'BIOLOGY', 'RIGHT');
            $saved = $this->OptionChoice_model->getOptionChoicesByName($name, 'BIOLOGY', 'RIGHT');
            if(isset($saved)){
                $this->session->set_flashdata('success-warning-message',"Option ".$name." has been created.");
                redirect(base_url().'BioOptionRight/index');
            }
        }
        else {
            $this->session->set_flashdata('error-warning-message',validation_errors());
            redirect(base_url().'BioOptionRight/create_new');
        }
    ...
    function update($id){
        ...
        if ($this->form_validation->run() ) {
            ...
            $this->session->set_flashdata('error-warning-message',validation_errors());
            redirect(base_url().'BioOptionRight/edit/'.$id);
        }
    ...
    function updatePriorities(){
        ...
        $n = $this->OptionChoice_model->setAllOptionChoicesPriority($ordered_optionIds, 'BIOLOGY','RIGHT');
    ...
    function delete($id){
        ...
        if($this->OptionChoice_model->deleteById($id, 'BIOLOGY','RIGHT')) {
            $this->session->set_flashdata('success-warning-message', "Right side option for Biological science has been deleted. ");
        }
        else{
